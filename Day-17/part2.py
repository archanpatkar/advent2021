...
def crossed(x,y,target=data):
    return True if x > target[0][1] or y < target[1][1] else False
    
initials = []
for x_v in range(200):
    for y_v in range(-200,200,1):
        if (x_v != 0 and y_v != 0) or ((x_v > 0 or x_v < 0) and y_v == 0) or (x_v == 0 and (y_v > 0 or y_v < 0)):
            initial_x = x_v
            initial_y = y_v
            curr_x_v = x_v
            curr_y_v = y_v
